...
class WC_Gateway_MicuentawebChoozeo extends WC_Gateway_MicuentawebStd
{
    ...
    {
        $this->id = 'micuentawebchoozeo';
        $this->icon = apply_filters('woocommerce_micuentawebchoozeo_icon', WC_MICUENTAWEB_PLUGIN_URL . 'assets/images/choozeo.png');
        $this->has_fields = true;
        $this->method_title = self::GATEWAY_NAME. ' - ' . __('Choozeo payment', 'woo-micuentaweb-payment');
    ...
    public function init_form_fields()
    {
        ...
        $this->form_fields['enabled']['description'] = __('Enables / disables Choozeo payment.', 'woo-micuentaweb-payment');

        $this->form_fields['title']['default'] = __('Payment with Choozeo without fees', 'woo-micuentaweb-payment');
        ...
        $this->form_fields['multi_options'] = array(
            'title' => __('CHOOZEO PAYMENT OPTIONS', 'woo-micuentaweb-payment'),
            'type' => 'title'
        );
        ...
        $this->form_fields['payment_options'] = array(
            ...
            'description' => __('Define amount restriction for each card.', 'woo-micuentaweb-payment')
        );
    }
        ...
        foreach ($options as $key => $option) {
            ...
            if (count($options) == 1) {
                echo '<input type="hidden"
                             id="micuentawebchoozeo_option_' . $key . '"
                             value="' . $key . '"
                             name="micuentawebchoozeo_option">';
            } else {
            ...
            echo '<label for="micuentawebchoozeo_option_' . $key . '" style="display: inline;">' . '
    ...
    public function process_payment($order_id)
    {
        ...
        $option = $_POST['micuentawebchoozeo_option'];
        $options = $this->get_available_options();
        $label = $options[$option]['label'];

        // Save selected payment option into session...
        set_transient('micuentawebchoozeo_option_' . $order_id, $option);
    ...
    protected function micuentaweb_fill_request($order)
    {
        ...
        $option = get_transient('micuentawebchoozeo_option_' . self::get_order_property($order, 'id'));
        $this->micuentaweb_request->set('payment_cards', $option);
        ...
        $this->micuentaweb_request->set('cust_country', 'FR');

        delete_transient('micuentawebchoozeo_option_' . self::get_order_property($order, 'id'));
    }
}
