...
class Micuentaweb_WC_Subscriptions_Subscriptions_Handler implements Micuentaweb_Subscriptions_Handler_Interface
{
    /**
     * {@inheritDoc}
     * @see Micuentaweb_Subscriptions_Handler_Interface::init_hooks()
     */
    public function init_hooks()
    {
        add_action('woocommerce_subscription_pending-cancel_micuentawebsubscription', array($this, 'cancel_subscription'));
        add_action('wcs_subscription_schedule_after_billing_schedule', array($this, 'update_subscription'));
    }

    /**
     * {@inheritDoc}
     * @see Micuentaweb_Subscriptions_Handler_Interface::cart_contains_subscription()
     */
    public function cart_contains_subscription($cart)
    {
        ...
        if ($order = WC_Gateway_MicuentawebStd::order_created_from_bo()) {
    ...
    /**
     * {@inheritDoc}
     * @see Micuentaweb_Subscriptions_Handler_Interface::cart_contains_multiple_subscriptions()
     */
    ...
    /**
     * {@inheritDoc}
     * @see Micuentaweb_Subscriptions_Handler_Interface::is_subscription_update()
     */
    ...
    /**
     * {@inheritDoc}
     * @see Micuentaweb_Subscriptions_Handler_Interface::get_parent_order()
     */
    ...
    /**
     * {@inheritDoc}
     * @see Micuentaweb_Subscriptions_Handler_Interface::subscription_info()
     */
    public function subscription_info($order)
    {
        $order_id = wcs_get_objects_property($order, 'id');

        $old_payment_method = get_transient('micuentawebsubscription_change_payment_' . $order_id);
        $is_payment_change = $old_payment_method ? true : false;
        delete_transient('micuentawebsubscription_change_payment_' . $order_id);

        // Payment method changes act on the subscription not the original order.
        if ($is_payment_change) {
            if ($old_payment_method === 'micuentawebsubscription') {
                return false;
            }
    ...
    /**
     * {@inheritDoc}
     * @see Micuentaweb_Subscriptions_Handler_Interface::process_subscription()
     */
    public function process_subscription($order, $response)
    {
        ...
        update_post_meta($subscription->get_id(), 'Subscription amount', WC_Gateway_Micuentaweb::display_amount($response->get('sub_amount'), $response->get('sub_currency')));
        update_post_meta($subscription->get_id(), 'Effect date', preg_replace('#^(\d{4})(\d{2})(\d{2})$#', '\1-\2-\3', $response->get('sub_effect_date')));

        if (WC_Gateway_Micuentaweb::is_successful_action($response)) {
    ...
    /**
     * {@inheritDoc}
     * @see Micuentaweb_Subscriptions_Handler_Interface::update_subscription()
     */
    public function process_subscription_renewal($order, $response)
    {
        ...
        if (wcs_get_objects_property($subscription, 'payment_method') !== 'micuentawebsubscription') {
            ...
            if (method_exists($subscription, 'set_payment_method')) {
                $subscription->set_payment_method($micuentaweb_subscription);
            } else {
                $subscription->payment_method = $micuentaweb_subscription->id;
                $subscription->payment_method_title = $micuentaweb_subscription->get_title();
            }
        ...
        if ($renewal_order->has_status('pending')) {
            try {
                ...
                $order_note = sprintf(_x('IPN subscription payment for recurrence #%s.', 'used in order note', 'woo-micuentaweb-payment'), $recurrence_number);
                $subscription->update_status('on-hold', $order_note);

                WC_Gateway_Micuentaweb::micuentaweb_add_order_note($response, $renewal_order);
                ...
                update_post_meta($renewal_order_id, 'Subscription amount', WC_Gateway_Micuentaweb::display_amount($response->get('amount'), $currency_code));
    ...
    /**
     * {@inheritDoc}
     * @see Micuentaweb_Subscriptions_Handler_Interface::cancel_subscription()
     */
    public function cancel_subscription()
    {
        ...
        $micuentaweb_subscription = new WC_Gateway_MicuentawebSubscription();
        $micuentaweb_subscription->cancel_online_subscription($subscription_id, $order_id);
    }
    ...
    public function update_subscription()
    {
        ...
        $micuentaweb_subscription = new WC_Gateway_MicuentawebSubscription();
        $micuentaweb_subscription->update_online_subscription($subscription_id, $order_id);
    }

    /**
     * {@inheritDoc}
     * @see Micuentaweb_Subscriptions_Handler_Interface::get_view_order_url()
     */
    ...
    /**
     * {@inheritDoc}
     * @see Micuentaweb_Subscriptions_Handler_Interface::get_subscription_statuses()
     */
    ...
    private function create_renewal_order($subscription)
    {
        ...
        if (is_wp_error($renewal_order)) {
            ...
            if (is_wp_error($renewal_order)) {
                throw new Exception(sprintf( __('Error: Unable to create renewal order with %s plugin.', 'woo-micuentaweb-payment'), WC_Gateway_Micuentaweb::GATEWAY_NAME));
            }
        }
