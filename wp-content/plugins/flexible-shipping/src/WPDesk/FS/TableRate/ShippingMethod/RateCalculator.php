...
class RateCalculator {
	...
	public function calculate_rate( MethodSettingsImplementation $method_settings, $rate_id, $user_logged_in ) {
		...
		if ( $add_method ) {
			 ...
			 */
			$shipping_contents = apply_filters( 'flexible_shipping_shipping_contents', $this->cart_contents, $method_settings->get_raw_settings(), $this->cart, $this->package );
	...
	private function prepare_meta_data( MethodSettingsImplementation $method_settings ) {
		$meta_data = array(
			...
			self::DESCRIPTION                      => wpdesk__( $method_settings->get_description(), 'flexible-shipping' ),
		);
