...
include("api.php");
define('click5_history_log_VERSION', '1.0.11');
...
function click5_after_switch_theme($data) {
  ...
  global $gCurrentUser;
  $table_name = $wpdb->prefix . "c5_history";
  if(esc_attr(get_option("click5_history_log_module_site_themes")) === "1")
...
function click5_registration_save( $user_id ) {
  ...
  if($isAdded)
  {
    if(esc_attr(get_option("click5_history_log_module_site_users")) === "1")
...
function click5_sbi_instagram_delete_source(){
  ...
  if(!empty($_REQUEST) && isset($_REQUEST['source_id'])){
    $username = $wpdb->get_results("SELECT username FROM ".$wpdb->prefix."sbi_sources WHERE id=".$_REQUEST['source_id']);
    if(!empty($username)){
    ...
    if(strpos($option,"polylang") !== false){
      ...
      foreach($value as $key => $index){
        ...
        if(is_array($index)){
          ...
          if(count($index) != 0 || count($old_value[$key]) != 0){
