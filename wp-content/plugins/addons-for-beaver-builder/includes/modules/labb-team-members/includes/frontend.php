...
foreach ($settings->team_members as $team_member):
    ...
    $child_output .= wpautop($wp_embed->autoembed(do_shortcode($team_member->member_position)));

    $child_output .= '</div>';

    $child_output .= '<div class="labb-team-member-details">';

    $child_output .= wpautop($wp_embed->autoembed(do_shortcode($team_member->member_details)));
